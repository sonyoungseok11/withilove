<?php
	...
	$parameter = "iUserStatus=$iUserStatus&search_Type=$search_Type&search_STR=$search_STR";
	
	$headerValArr = array("<input type=\"checkbox\" class=\"allCheck\" />","No.","M레벨","H레벨", "ID", "이름", "상태", "Type",$print_day, "접속여부","수정");
	$headerColArr = array("4%","5%","6%","6%","auto","20%","8%","8%","15%","7%","8%");
	...
	$TbSQL = "
		SELECT u.id, u.iMLevel, u.iHLevel, u.sUserId, u.sUserName, u.eUserType, u.iUserStatus, DATE_FORMAT(u.dInDate,'%Y-%m-%d') AS dInDate, DATE_FORMAT(u.dOutDate,'%Y-%m-%d') AS dOutDate,
		l.user_id AS logid, l.iActive, l.dLogDate
		FROM `users` u
		LEFT JOIN `user_login_log` l ON u.id = l.user_id 
	 ". $where . $limit ." 
	";
	$Data = $db_conn -> sqlShowData_adminUserList($default_db, $TbSQL, $TablePositionArr, $st_num);
