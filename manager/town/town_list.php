...
$iActive = isset($iActive) ? $iActive : '1';
...
if ($group1_id) {
	...
	if ($gCount > 0) {
		...
		$g2Select = "
			<select name=\"group2_id\" class=\"auto_submit_select\">
...
// 모든 학원수 가지고 오기
$ATCSQL = "SELECT COUNT(id) FROM `town_academy` WHERE iActive='".$iActive."'";
...
$HeaderVal = array(
	...
	array("title" =>"연락처",	'col' => '15%',		'pos' => 'center',	'sort' => ""),
	array("title" =>"홈페이지",	'col' => '15%',		'pos' => 'center',	'sort' => ""),
	array("title" =>"주소",	'col' => '15%',		'pos' => 'left',	'sort' => ""),
	array("title" =>"명령",	'col' => '10%',		'pos' => 'center',	'sort' => ""),
);
$caption = "
...
if ($iTotal > 0) {
	...
	$SSQL = "
		SELECT id, user_id, sName, sCeo, sHphone, sAddr, sAddrSub, sUrl FROM `town_academy`
		".$WhereIs . $Order. $limit ." 
	";
	$tbody = sqlShowData_town_manager($SSQL, $st_num);
} else {
...
switch($iActive) {
	case '0':
		$act['title'] = "우리동네학원 대기학원정보";
		break;
	default :
		$act['title'] = "우리동네학원 등록학원정보";
		break;
}
?>
