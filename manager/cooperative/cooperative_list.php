...
include_once ("$path/inc/gnb.php");
	...
	$FSQL ="
			...
			WHERE l.eType='".$eType."' AND f.eType='L' ORDER BY l.iSort ASC
			";
	...
	while ($row = mysql_fetch_array($FRES)) {
		...
		$data .= "	<span class=\"command\">
						<a href=\"javascript:;\" onclick=\"Cooperative.ShowDetail(".$row['id'].")\" class=\"jButton small\">보기</a>
...
.cooperative_list .logo {width:200px}
...
.cooperative_list .sortable li {height:50px; position:relative; display:table; width:100%; box-sizing:border-box;}
.cooperative_list .sortable li > span {display:table-cell; text-align:center; vertical-align:middle;}
.cooperative_list .sortable .ui-icon {display:inline-block; top:16px; position:absolute; left:8px;}
.cooperative_list .sortable .logo{width:200px;}
