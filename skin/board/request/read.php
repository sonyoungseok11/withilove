...
if ($MEMBER['iMLevel'] == 1) {
	...
	if ($wish_count > 0) {
		$wishlist .="<ul>";
		$WSQL = "
			SELECT u.id, sUserId, sUserName FROM `board_user_wishlist` w
		...
		$WRES = $db_conn -> sql($default_db, $WSQL);
			while ($WISHLIST = mysql_fetch_assoc($WRES)) {
				$wishlist .="<li>";
				$wishlist .="<span>".$WISHLIST['sUserName']."(".$WISHLIST['sUserId'].")</span>";
				$wishlist .="</li>";
			}
		$wishlist .="</ul>";
	}
...
<div class="command">
	...
	if ($Datas['DiffDay'] > 0) {					
		if ($MEMBER['id']) {
			...
			$ReturnValue .="	<span class=\"ask\"><a href=\"javascript:;\" onclick=\"needMember();\" class=\"msButton medium pink\">신청</a></span>\n";
		}
	} 
	echo $ReturnValue;
	?>
